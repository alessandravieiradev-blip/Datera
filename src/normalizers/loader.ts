...
import { KeyNormalizer, registerKeyNormalizer } from "./registry";
...
export function loadNormalizerModules(modulePaths: string[]): void {
    for (const modulePath of modulePaths) {
        ...
        try {
            ...
            throw new Error(
                `Não consegui carregar o módulo de normalizadores "${modulePath}": ${reason}`,
            );
        }

        const normalizers = (
            loaded as { normalizers?: Record<string, KeyNormalizer> } | null
        )?.normalizers;
        ...
        for (const [name, normalizer] of Object.entries(normalizers)) {
            if (typeof normalizer !== "function") {
                throw new Error(
                    `Em "${modulePath}", "${name}" não é uma função.`,
                );
            }
