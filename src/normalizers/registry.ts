...
export function registerKeyNormalizer(
    name: string,
    ...
    if (existing && existing !== normalizer) {
        throw new Error(
            `Já existe um normalizador de chave registrado com o nome "${name}".`,
        );
    }
...
export function getKeyNormalizer(name: string): KeyNormalizer {
    ...
    if (!normalizer) {
        ...
        throw new Error(
            `Normalizador de chave "${name}" não encontrado. Disponíveis: ${hint}`,
        );
    }
    return normalizer;
}
