...
describe("MergeFilter", () => {
  it("mescla 2 linhas duplicadas: overwrite fica com o último valor", () => {
    ...
    const filter = new MergeFilter("codigo", [{ column: "email", strategy: "overwrite" }]);
  ...
  it("mescla 3+ linhas duplicadas com concat, sem repetir valor igual", () => {
    ...
    const filter = new MergeFilter("codigo", [{ column: "obs", strategy: "concat" }]);
  ...
  it("extra-column joga o telefone excedente em telefone_2, telefone_3...", () => {
    ...
    const filter = new MergeFilter("codigo", [{ column: "telefone", strategy: "extra-column" }]);
...
describe("MergeFilter com normalizador de chave", () => {
  it("sem normalizador, mantém o comportamento antigo (só trim)", () => {
    ...
    const filter = new MergeFilter("sku", [{ column: "cor", strategy: "concat" }]);
  ...
  it("chave suja com texto colado agrupa com a versão limpa", () => {
    ...
    const filter = new MergeFilter("sku", [{ column: "cor", strategy: "extra-column" }], {
      keyNormalizer: (raw) => {
        const m = String(raw).toUpperCase().match(/([A-Z]+)\W*(\d+)/);
  ...
  it("chave rejeitada vira 'sem chave': isolada, no fim, com rótulo próprio", () => {
    ...
    expect(result[1]).toMatchObject({ id: 1, sku: "sku inválido", cor: "azul" });
    expect(result[2]).toMatchObject({ id: 3, sku: "sku inválido", cor: "rosa" });
  });

  it("rótulo padrão de rejeitada usa o nome da coluna-chave", () => {
    const filter = new MergeFilter("matricula", [], { keyNormalizer: () => null });
  ...
  it("valor vazio nem chega no normalizador e usa o rótulo de chave vazia", () => {
    ...
    expect(result.map((r) => r.email)).toEqual(["A@x.com", "sem email", "sem email"]);
  });
  ...
  it("aceita normalizador registrado por nome e reclama de nome desconhecido", () => {
    ...
    expect(filter.apply([{ sku: "ab1", cor: "a" }, { sku: "AB-1", cor: "b" }])).toHaveLength(1);
    expect(() => new MergeFilter("sku", [], { keyNormalizer: "nao-existe" })).toThrow(/nao-existe/);
  });

  it("registrar dois normalizadores diferentes com o mesmo nome dá erro", () => {
    registerKeyNormalizer("nome-repetido", () => null);
    expect(() => registerKeyNormalizer("nome-repetido", () => null)).toThrow();
  });
});
...
describe("MergeFilter: collapse-column nas linhas sem chave", () => {
  const columns = [
    {
      ...
      unkeyed: { strategy: "collapse-column" as const, into: "Nomes sem chave", separator: " | " },
    },
  ];
  ...
  it("junta a coluna de todas as linhas sem chave numa coluna só, numa linha só", () => {
    ...
    expect(result[1]).toEqual({ ref: "(sem ref)", "Nomes sem chave": "Bia | Duda | Edu" });
  ...
  it("chave vazia e chave rejeitada ficam em linhas separadas, cada uma com seu rótulo", () => {
    ...
    const filter = new MergeFilter("ref", columns, {
      keyNormalizer: (raw) => (/^[A-Z0-9]+$/.test(String(raw)) ? { key: String(raw) } : null),
      emptyKeyLabel: "vazio",
  ...
  it("sem 'into', o nome da coluna nova sai de coluna + rótulo; separador padrão é '; '", () => {
    const filter = new MergeFilter("ref", [
      { column: "nome", strategy: "concat", unkeyed: { strategy: "collapse-column" } },
    ]);
    ...
    expect(result).toEqual([{ ref: "(sem ref)", "nome (sem ref)": "Ana; Bia" }]);
  });

  it("funciona pra mais de uma coluna ao mesmo tempo", () => {
    const filter = new MergeFilter("ref", [
      { column: "nome", strategy: "concat", unkeyed: { strategy: "collapse-column", into: "Nomes" } },
      { column: "email", strategy: "concat", unkeyed: { strategy: "collapse-column", into: "Emails" } },
    ]);
    ...
    expect(result[0]).toMatchObject({ Nomes: "Ana; Bia", Emails: "a@x.com" });
  });

  it("sem unkeyed configurado, linhas sem chave continuam uma por linha (comportamento antigo)", () => {
    const filter = new MergeFilter("ref", [{ column: "nome", strategy: "extra-column" }]);
